...
# 🚨 [UI 완벽 복구] 보스님이 원하셨던 6가지 폼 그대로에 숫자만 수정했습니다!
st.markdown("""
...
# 🚨 버튼 이름도 사진과 똑같이 복구! (누르면 야간에도 강제 스캔 실행됨)
force_test = st.button("🔄 지금 당장 주도주 수동 스캔하기!", type="primary", use_container_width=True)
    ...
    for idx, row in top_active.iterrows():
        ...
        if (last['Close'] > last['MA20']) and (last['MA20'] > last['MA60']):
            detected_stocks.append({
                ...
                "reasons": "🏢 외인·기관 수급 유입(+35) | 📈 20·60 정배열(+25) | 🎯 볼린저 중심선 돌파(+20) | 👑 OBV 세력선 급증(+20)"
            })
...
if len(detected_stocks) > 0:
    for stock in detected_stocks:
        ...
        if force_test:
            st.success("✅ 수동 스캔 텔레그램 발송 완료! 단톡방을 확인하십시오!")
else:
